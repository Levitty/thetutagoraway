...
| **Content system** | Pedagogical schema (worked example + scaffolded steps + hint ladder + misconception feedback + variety + verify hook); quality gate that **independently verifies answers** and validates teaching-visual models. |
| **Teaching layer** (2026-07-29) | The lesson now *teaches* instead of quizzing: **learner-paced worked examples** (one step at a time, guess-then-reveal, self-explanation nudge), **completion problem** after the example (steps shown, student finishes — Renkl fading), **named misconception feedback** when a wrong answer matches an authored error (was authored on every skill but never surfaced), the **full 3-level hint ladder**, and **visual teaching models** (`TeachingVisual.jsx`): equations as a balance scale (per-step states), integer/temperature change as number-line jumps, expansion as area models, fractions as bar models. Practice views hide exactly the answer (empty cells, `?` landing point, unshaded bar + guide line); reveals complete the picture. Dual coding + CPA, wired through `model` fields on problems and solution steps. Extended same-day with: **fraction-of-a-fraction grids** (multiply), **re-cut bar models** for unlike denominators (the WHY of common denominators, capped at countable LCDs), **percent bars**, **labeled shape diagrams** (rect/triangle/circle, dashed-around vs shaded-inside), and **bounds intervals** for rounded measurements — coverage now spans ~30 skills via the content registry. Round 3: **place-value charts** for decimal alignment (with carry row), **pattern-growth towers** for arithmetic sequences, **two-colour ratio bars** (G6_RATIOS newly authored — 104 authored skills), and the balance scale becomes an **interactive manipulative** on the concrete (CPA) modality — the student performs the legal moves themselves; working it to completion books the solve as scaffolded, never clean. |
| **Symbolic layer** | Mini-CAS (expression trees, evaluate, differentiate) powering the hard calculus with verified numeric answers. |
| **Authored content** | **171 skills, all gate-verified** across G1–12: Numbers, Algebra, Fractions, Geometry, Measurement, Statistics & Calculus — incl. the CBC-gap topics Temperature (G7), Approximations & Errors (G9), ratio sharing (G6), the full Grade 1–4 early-primary foundation, and — 2026-07-29 — the complete Cambridge Lower Secondary gap fill (29 skills: place value to hundred-millions, divisibility, prime factorisation, decimal PV, extended squares, standard form, proportion, profit/loss, compound interest, surds, variation, inequalities incl. the negative-divide flip, the Stage-8 straight-line spine, parallel-line angles, congruence tests, similarity, constructions, bearings, advanced conversions, composite area, surface area, prism volume, pie-chart arithmetic, tree-diagram probability, grouped data). **Cambridge in-scope coverage: 72/72 = 100%.** |
| **Curriculum graph** | **244 math skills G1–12** (+ ACCA AFM 87, APM 80). Structurally clean (no broken refs/cycles). Grades 1–4 added 2026-07-29: 38 early-primary skills (counting, ten-frame add/sub, base-ten place value, column methods, tables/arrays, sharing/remainders, unit fractions, clocks, Kenyan money, shapes/turns/symmetry, scaled charts), every one authored with its manipulative and wired as prerequisites of the Grade-5 foundation. |
...
| Area | Gap |
|---|---|
| **Content coverage** | 171/244 math skills authored to the bar (Cambridge view 100%, CBC view 86%). The rest still use legacy generators (correct but thin). |
